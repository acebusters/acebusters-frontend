...
import { apiBasePath, tokenContractAddress } from '../../app.config';
...
export function getBalance(address) {
  ...
  const payload = new Promise((resolve, reject) => {
    contract.balanceOf(address, (e, a) => {
      if (e) {
        reject(e);
      }

      const balance = a.toNumber() / (10 ** baseUnit.toNumber());
      resolve({ balance });
    });
  });
...
export function getTables() {
  ...
  const payload = fetch(request).then((res) => res.json(), (err) => err);
